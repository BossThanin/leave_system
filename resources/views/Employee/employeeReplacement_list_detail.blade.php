...
<?php
    ...
    $query = "SELECT `leave`.reason, positions.position_name, employee.position_id, employee.id, employee.first_name, employee.last_name, leavetype.leavetype_name as leave_type, `leave`.starts_date, `leave`.end_date, `leave`.comment, `leave`.`image`
        FROM `leave`
    ...
    while($row = $result->fetch_assoc()){
        ...
        $options_reason .= "<option value='" . $row['reason'] . "'>" . $row['reason'] . "</option>";
    }
    $conn->close();
?>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="col-md-7">
                                                    <div class="form-group">
                                                        <label for="">เหตุผลการลา</label>
                                                        <p class="form-control" rows="3" placeholder="กรอกที่นี่..." style="height: 99px;" <?php echo $options_reason; ?> ></p>
                                                    </div>
                                                </div>
                                            </div>
                                            <!-- อัพโหลดไฟล์หลักฐาน -->
